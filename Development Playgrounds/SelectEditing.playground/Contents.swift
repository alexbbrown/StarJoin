...
func testSelectionConstructionZero() {
    ...
    let mySelection = Selection<SKNode>.selection(parent: scene, nodes: scene.childNodes, data: emptyData)

    assert(0 == mySelection.nodes.count)
...
func testSelectionConstructionOne() {
    ...
    assert(0 == mySelection.nodes.count) // the join has NO nodes to begin with (before enter)
